...
import { getDistrictRankingAllCases } from "@/lib/reportTemplates";
...
export default async function DistrictRankingAllCasesPage({
  ...
  const { rows, totalRow } = getDistrictRankingAllCases(db, selectedPeriodIds.length > 0 ? selectedPeriodIds : undefined);
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="no-print flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <h1 className="mt-1 text-lg font-semibold text-slate-900">District Ranking - All Cases</h1>
          <p className="mt-1 text-sm text-slate-500">District ranking across every classified-case category.</p>
        </div>
        <div className="flex gap-2">
          <a href={`/api/report-templates/district-ranking-all-cases/export?${exportQuery.toString()}`}>
      ...
      <Card>
        <CardHeader
          title="District Ranking - All Cases"
          ...
          <table className="w-full text-left text-sm">
            <thead className="border-b border-slate-100 text-xs uppercase text-slate-400">
              <tr>
                ...
                <th className="px-4 py-2 font-medium">Rank in all cases</th>
              </tr>
            </thead>
