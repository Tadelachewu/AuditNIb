// Shared bulk-insert logic: given a `Database`-shaped object (whichever
// module built it - the one-time JSON import in migrate-from-json.ts, or a
// brand-new install's buildSeedDatabase() in seedData.ts), inserts every row
// into Postgres via `createMany()`, in dependency order. Only ever meant to
// run once against an EMPTY set of tables - a second run against non-empty
// tables will fail on id/unique-constraint collisions (createMany does not
// upsert). See prisma/clear-partial-etl.ts for clearing a partial run first.
...
export async function insertDatabaseIntoPostgres(prisma: PrismaClient, db: Database): Promise<void> {
  ...
  await prisma.user.createMany({
    data: db.users.map((u) => ({
      ...
      passwordExpiresAt: toDateOrNull(u.passwordExpiresAt),
